...
router.post("/messages", verifyToken, async (req, res) => {
  try {
    ...
    console.log('Creating chat message with sender ID:', req.userId);
    let chatMessage = new chatModel({
      message,
      sender: req.userId, // from verifyToken
    });

    await chatMessage.save();

    chatMessage = await chatMessage.populate("sender", "name username image_url"); // ✅ populate here too

    // Broadcast to socket if you want (optional)
...
router.get("/messages", verifyToken, async (req, res) => {
  try {
    const messages = await chatModel
      .find()
      .populate("sender", "name username image_url") // ✅ populate sender details
      .sort({ timestamp: -1 })
      .limit(100);
